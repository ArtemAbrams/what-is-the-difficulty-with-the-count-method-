...
public static int Count<TSource>(this IEnumerable<TSource> source) 
{
    ...
    /* Складність метода полягає у тому що він намагається переобразувати вхідну колекцію в ICollection.
     При успішному перетворенні методу повертається значення властивості Count.
     У разі невдачі потрібно об’єднати всю колекцію для вирахування кількості елементів, внизу як раз бачимо як у нас перебираються елементи */  
...
/* Взагалі краще використовувувати властивість count ніж метод count(), якщо все буде успішно і наш IEnumerable імплементує ICollection
   то час виконання алгоритму методу буде О(1) що не буде різнитися з властивістю count, а але в крайньому випадку може виконатися за час  О(n)*/ 